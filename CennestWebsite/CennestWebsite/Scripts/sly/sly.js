	...
	function Sly(frame, options, callbackMap) {
		...
		function slideTo(newPos, immediate, dontAlign) {
			...
			if (itemNav && dragging.released && !dontAlign) {
				...
				var center = tempRel.centerItem;
				var first = tempRel.firstItem;
				var last = center + 1;
				last = tempRel.centerItem === tempRel.lastItem ? tempRel.lastItem : last;
				$items.each(function (i, element) {
				    if (i === center || i === first || i === last || i > first && i < center) {
			...
			if ($handle.length) {
				...
				if (last.hPos !== hPos.cur) {
					...
					if (transform) {
					    ...
					    if (hPos.cur === 0) {
					        $frame.css('margin-left', '20px!important');
					    } else {
